...
from delivery_gui import DeliveryWindow  # Ensure this matches the path to your delivery_gui.py file
...
class OrderWindow:
    def __init__(self):
        ...
        if "00:00" <= current_time <= "11:30":
            ...
            self.menu = load_menu('menuitems/Breakfast.csv')
        elif "11:30" < current_time <= "23:59":
            self.meal_type = 'lunch'
            self.menu = load_menu('menuitems/Lunch.csv')
        else:
    ...
    def show_summary_view(self):
        ...
        for idx, order in enumerate(self.all_orders, start=1):
            subtotal = self.calculate_order_total(order)
            overall_total += subtotal
            summary_text += f"Order #{idx}:\n"
            summary_text += f"  Main: {order['main']} ({order['size']})\n"
            summary_text += f"  Side: {order['side']}\n"
            summary_text += f"  Daily Special: {order.get('special', 'None')}\n"
            summary_text += f"  Customization: {order.get('customization', 'No customization')}\n"
            summary_text += f"  Beverage: {order.get('beverage') or 'None'}\n"
